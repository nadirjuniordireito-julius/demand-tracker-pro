...
export type TEPropsWithLabels = Omit<TEProps, "labels"> & { labels: TELabels };
 ...
 */
export function TermoEncerramentoReportInner(props: TEPropsWithLabels) {
  const {
    PROJETO_COD_TED,
    PROJETO_NOME,
    DEMANDA_CODIGO,
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page} wrap>
        ...
        <Text style={styles.sectionTitle}>{L.resultsDelivered}</Text>
        <View style={styles.box}>
          <Text>{RESULTADO_ENTREGUE}</Text>
        </View>
...
export default function TE(props: TEProps) {
  const { t } = useTranslation();
  const { labels: L } = props;

  const resolvedLabels: TELabels = L
    ? { ...L }
    : {
        documentTitle: t("closingTerm.report.documentTitle"),
        demandNumber: t("closingTerm.report.demandNumber"),
        planningDate: t("closingTerm.report.closingDate"),
        resultsDelivered: t("closingTerm.report.resultsDelivered"),
        costs: t("closingTerm.report.costs"),
        signature: t("closingTerm.report.signature"),
        signatureNote: t("closingTerm.report.signatureNote"),
        signerName: t("closingTerm.report.signerName"),
        signerRole: t("closingTerm.report.signerRole"),
        pageOf: (p: number, tot: number) => t("closingTerm.report.pageOf", { page: p, total: tot }),
      };

  return <TermoEncerramentoReportInner {...props} labels={resolvedLabels} />;
}
